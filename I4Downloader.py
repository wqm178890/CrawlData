...
def select_paper_data(start, end):
    try:
        ...
        count = cur.execute('select detail_url, source_url, category, data_h from is_wallpaper where data_h >= 900 limit %d,%d' % (start, end))
...
root_path = "G:/is_wallpaper"
start_index = 2115
def run(start):
    i = start
    try:
        results = select_paper_data(i, 100000)
